...
class PylintCommand(setuptools.Command):
    ...
    def finalize_options(self):
        self.lint_packages = [package.strip() for package in re.split("[\s,]+", self.lint_packages) if package != ""]
        self.lint_exclude_packages = [module.strip() for module in re.split("[\s,]+", self.lint_exclude_packages)]
        if self.lint_output:
